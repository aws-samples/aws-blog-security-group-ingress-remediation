...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    compliant = 'EC2 Instance {} has no unjustified open ports'
    non_compliant = 'EC2 Instance {} has public ports {} without a valid exception'

    ec2_instance = json.loads(event['invokingEvent'])['configurationItem']['configuration']
    ...
    if unjustified_open_ports:
        return build_evaluation_from_config_item(
            configuration_item,
            'NON_COMPLIANT',
            annotation=build_annotation(non_compliant.format(instance_id, unjustified_open_ports))
        )
    else:
        return build_evaluation_from_config_item(
            configuration_item,
            'COMPLIANT',
            annotation=build_annotation(compliant.format(instance_id))
        )
